...
/**
 * Action ShootFlagHolderWithPrefs for Yaposh.
 *
 * @author Jimmy
 * 
 ...
 * environment.
 */
@PrimitiveInfo(name = "ShootFlagHolderWithPrefs", description = "Shoot at the bot carrying a $team flag using weapon preferences.")
public class ShootFlagHolderWithPrefs<CONTEXT extends CTFBotContext> extends ParamsAction<CONTEXT> {

    /**
     * Constructor of the action, used during automatic instantiation.
     */
    public ShootFlagHolderWithPrefs(CONTEXT ctx) {
        super(ctx);
    }
    ...
    public ActionResult run(@Param("$team") Team team) {
        ctx.getLog().warning("ACTION: ShootFlagHolderWithPrefs[team=" + team + "]");
