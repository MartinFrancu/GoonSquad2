...
/**
 * Action ShootPlayerWithPrefs for Yaposh.
 *
 * @author Jimmy
 * 
 ...
 * environment.
 */
@PrimitiveInfo(name="ShootPlayerWithPrefs", description="Starts shooting a nearest enemy player, if is visible, using weapon preferences.")
public class ShootPlayerWithPrefs<CONTEXT extends CTFBotContext> extends ParamsAction<CONTEXT> {

    public ShootPlayerWithPrefs(CONTEXT ctx) {
        super(ctx);
    }
    ...
    public ActionResult run() {
        if (ctx.getPlayers().canSeeEnemies()) {
            ...
            ctx.getLog().warning("ACTION: ShootPlayerWithPrefs");
            ctx.getLog().warning("  +-- Shooting at " + player.getName() + ".");
            ctx.getShoot().shoot(ctx.getWeaponPrefs(), player);
            ctx.isShooting = true;
        } else {
            ctx.getLog().warning("ACTION: ShootPlayerWithPrefs");
